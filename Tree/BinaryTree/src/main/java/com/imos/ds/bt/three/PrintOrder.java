...
public class PrintOrder {
    ...
    public List<Node> printLevelTransverse(Node node, int level) {
        if (node == null) {
            return createNewArray();
        }
        if (level == 0) {
//            System.out.println(node.value);
            List<Node> list = createNewArray();
            list.add(node);
            return list;
        }
        List<Node> list = createNewArray();
        if (node.left != null) {
            list.addAll(printLevelTransverse(node.left, level - 1));
        }
        if (node.right != null) {
            list.addAll(printLevelTransverse(node.right, level - 1));
        }
        return list;
    }
